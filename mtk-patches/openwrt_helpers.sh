...
# === YOUR PROTOCOL FIX START ===
	log_info "Enforcing stable protocols in generated feeds configuration..."
	# 1. Force git:// for all OpenWrt links
	sed -i 's|https://git.openwrt.org|git://git.openwrt.org|g' "${ab_tmp}/feeds.conf.mtk"
	
	# 2. Specifically revert the 'video' feed back to https://
	sed -i '/video/s|git://|https://|g' "${ab_tmp}/feeds.conf.mtk"
	
	# 3. Ensure any existing feeds.conf is removed so .default is used
	[ -f "${openwrt_root}/feeds.conf" ] && rm -f "${openwrt_root}/feeds.conf"
	# === YOUR PROTOCOL FIX END ===
